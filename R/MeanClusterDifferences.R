...
MeanClusterDifferences <- function(Object)
{
  ...
  Cluster.Combo <- cbind(Cluster.Combo,
                         matrix(NA, nrow = nrow(Cluster.Combo), ncol = 5))
 colnames(Cluster.Combo) <- c("Cluster_A", "Cluster_B",
                              "Cluster_A_Mean", "Cluster_B_Mean",
                              "Diff_A-B", "Diffs_A", "Diffs_B")
  
  for(cl.i in 1:Ncluster)
  {
    idx_B <- Object@Data[[1]][Object@Data[[1]]$cluster == cl.i, "RR_name"]
    
    B.diffs.vc <- vector()
    for(A.i in 1:length(idx_B))
    {
      for(B.i in 1:length(idx_B))
      {
        if(A.i == B.i){next()}
        B.diffs.vc <- c(B.diffs.vc,
                        abs(rowDiffs(as.matrix(RR.Data[c(idx_B[A.i],idx_B[B.i]),c(idx_B[A.i],idx_B[B.i])]))))
      }
    }
    Cluster.Combo[Cluster.Combo[,"Cluster_A"] == cl.i,"Diffs_A"] <- mean(as.numeric(B.diffs.vc))
    Cluster.Combo[Cluster.Combo[,"Cluster_B"] == cl.i,"Diffs_B"] <- mean(as.numeric(B.diffs.vc))
  }
  for(cl.i in 1:nrow(Cluster.Combo))
  {
    ...
    for(A.i in 1:length(idx_A))
    {
      ...
      {
        diffs.vc <- c(diffs.vc,
                      abs(rowDiffs(as.matrix(RR.Data[c(idx_A[A.i],idx_B[B.i]),c(idx_A[A.i],idx_B[B.i])]))))
      }
    }
